...
import datagenerator.generators.CustomWieghtGenerator;

/**
 * The CustomWieghtField is the Field object that is used by the MainGUI. The CustomWieght field stands for
 * Custom Wieghted User Data Field. This is a Field Class that generates randomly selected data from the  
 * a wieghted dataset supplied by the end user. The first column of a csv file is the data and the Second Column
 * is the corresponding wieght for each value.
 * @author Patrick McMorran
 */
public class CustomWieghtField implements Field{
	...
	CustomWieghtGenerator Generator;

	datagenerator.gui.Field_GUI GUITUPLE; //This is the row that is in the main window
	datagenerator.fields.config.Config_CustomWieght Config;
	 ...
	 */
	public CustomWieghtField(){
		Config = new datagenerator.fields.config.Config_CustomWieght(this);
	}
	 ...
	 */
	public CustomWieghtField(CustomWieghtCompressedField Comp){
		Config = new datagenerator.fields.config.Config_CustomWieght(this, Comp);
	 ...
	 * @param File File This is a String that contains the file path to the CSV file where the data the generator will
	 * use is located.
	...
	public void Create(Connection c) {
		conn = c;
		this.Generator = new CustomWieghtGenerator(conn, FilePath, unique);
	...
	public CompressedField getConfig() {
		return new CustomWieghtCompressedField(Name, activated, unique, FilePath);
	}
